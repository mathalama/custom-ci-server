...
public class EmailNotificationService implements NotificationService {
    ...
    @Value("${zovik.notification.email.enabled:false}")
    private boolean emailEnabled;

    @Value("${zovik.notification.email.from:}")
    ...
    public void handleBuildCompleted(BuildCompletedEvent event) {
        ...
        try {
            ...
            helper.setSubject(String.format("Zovik: Build #%d [%s]", event.buildId(), event.status().name()));
